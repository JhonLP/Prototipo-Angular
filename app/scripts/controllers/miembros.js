...
angular.module('prototipoApp')
  .controller('MiembrosCtrl', function ($scope,$http,$modal,$filter) {
  	$http.get('http://localhost:9000/miembros.json').success(function(data){
  	$scope.miembros = data;
  	});
  	$scope.gridOptions = {
  		data: 'miembros',
  		showGroupPanel:true, //permite agrupar por categorias
  		showFilter:true, // añade un campo de busqueda
      ...
      modalInstance.result.then(function(selectedItem){
        $scope.miembros.push({
          ...
          fechaUnion: $filter('date') ($scope.nuevoMiembro.fechaUnion, 'dd/MM/yy')
        });
      });
  	};
  })
